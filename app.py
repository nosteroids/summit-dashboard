...
class MetaAPI:
    ...
    def get_campaign_budgets(self, account_id: str, campaign_ids: list) -> dict:
        """
        For each campaign ID, fetch ACTIVE ad sets only and sum their daily_budget.
        Returns a dict: { campaign_id: total_daily_budget_float }
        """
        result = {}
        for cid in campaign_ids:
            url = f"{BASE_URL}/{cid}/adsets"
            params = {
            ...
            try:
                while url:
                    ...
                    for adset in data.get("data", []):
                        db = parse_float(adset.get("daily_budget", 0))
                        lb = parse_float(adset.get("lifetime_budget", 0))
                        if db > 0:
                            total_daily += db / 100.0
                        elif lb > 0:
                            has_lifetime = True
...
def render_trend_chart(period_summaries: list):
    """
    Renders a combo chart: bars for Spend (left axis), bars for Leads (right axis),
    line for CPL (far right axis). Each metric has its own scale so labels are readable.
    """
    ...
    fig.add_trace(go.Bar(
        ...
        textfont=dict(size=12, color="#4A90D9"),
        offsetgroup=1,
    ))

    # Bar: Leads (right axis y2)
    fig.add_trace(go.Bar(
        name="Leads",
        x=labels,
        y=leads,
        ...
        textposition="outside",
        textfont=dict(size=12, color="#2e8b57"),
        offsetgroup=2,
    ))

    # Line: CPL (far right axis y3)
    fig.add_trace(go.Scatter(
        ...
        textfont=dict(size=12, color="#E05C2A"),
    ))

    # Calculate axis ranges with padding so outside labels aren't clipped
    spend_max  = max(spends)  if max(spends)  > 0 else 1
    leads_max  = max(leads)   if max(leads)   > 0 else 1
    cpl_max    = max(cpls)    if max(cpls)    > 0 else 1
    cpl_min    = min([c for c in cpls if c > 0], default=0)
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text="Spend ($)", font=dict(size=12, color="#4A90D9")),
            showgrid=True,
            gridcolor="#f0f0ee",
            zeroline=False,
            tickfont=dict(size=11, color="#4A90D9"),
            range=[0, spend_max * 1.25],
        ),
        yaxis2=dict(
            title=dict(text="Leads", font=dict(size=12, color="#2e8b57")),
            overlaying="y",
            side="right",
            showgrid=False,
            zeroline=False,
            tickfont=dict(size=11, color="#2e8b57"),
            range=[0, leads_max * 1.25],
            anchor="x",
        ),
        yaxis3=dict(
            title=dict(text="CPL ($)", font=dict(size=12, color="#E05C2A")),
            overlaying="y",
            side="right",
            showgrid=False,
            zeroline=False,
            tickfont=dict(size=11, color="#E05C2A"),
            range=[max(0, cpl_min * 0.8), cpl_max * 1.3],
            anchor="free",
            position=1.0,
        ),
    )
    ...
    for tab, period, summary in zip(tabs, periods, period_summaries):
        with tab:
            ...
            if not camp_data:
                ...
                # Fetch daily budgets for all campaigns in this period (cached)
                budget_key = f"budgets_{acc['id']}"
                if budget_key not in st.session_state:
                    cids = [c.get("campaign_id") for c in camp_data if c.get("campaign_id")]
                    st.session_state[budget_key] = api.get_campaign_budgets(acc["id"], cids)
                budget_map = st.session_state[budget_key]
                ...
                for c in sorted(camp_data, key=lambda x: -parse_float(x.get("spend", 0))):
                    ...
                    bval = budget_map.get(cid)
                    if bval is None:
                        budget_cell = '<span style="color:#aaa;">—</span>'
                    elif bval == -1.0:
                        budget_cell = '<span style="color:#888;font-size:12px;">Lifetime</span>'
                    else:
                        budget_cell = f'<span style="font-weight:600;">${bval:,.2f}/day</span>'
                ...
                <table class="camp-table">
                  <thead>
                    <tr>
                      ...
                      <th class="right">Daily Budget</th>
                    </tr>
                  </thead>
                  <tbody>{''.join(rows_html)}</tbody>
                </table>"""
